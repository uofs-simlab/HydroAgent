...
def load_api_key() -> str:
    """Resolve the OpenAI API key from env, a local .env, or the SYMFLUENCE file."""
    ...
    fallback = HERE.parent / "SYMFLUENCE" / "apikey"
    if fallback.exists():
    ...
    raise SystemExit(
        "No OpenAI API key found. Set OPENAI_API_KEY, create a .env file, "
        "or add the key to ../SYMFLUENCE/apikey."
    )
...
def prepare_query(
    ...
    *,
    ...
    if force_config or is_config_query(text):
        ...
        if not config:
            raise SystemExit("Config query detected but no KEY: value fields were parsed.")
...
def load_prompts() -> List[Document]:
    """Read every prompt file in ``prompts/`` into one or more LangChain Documents."""
    ...
    if not files:
        raise SystemExit(
            f"No prompt files (*.md / *.txt) found in {PROMPTS_DIR}. Add some first."
        )
    ...
    if not docs:
        raise SystemExit(f"No prompts could be parsed from files in {PROMPTS_DIR}.")
    return docs
